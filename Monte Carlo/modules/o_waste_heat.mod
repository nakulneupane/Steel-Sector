...
# mip-v3 CCS-steam competition: the ACCESSIBLE pool (recoverable gas x WHR
# penetration n9_whr) is allocated between power generation and CCS solvent-
# regeneration steam. CCS steam therefore costs the FOREGONE WHR POWER CREDIT
# (opportunity cost) rather than a posted price; the gas-fired backup boiler in
# q_carbon_capture.mod covers any regen demand beyond the pool. Heat->steam
# recovery (whr_steam_eff ~0.85) is far more efficient than heat->power
# (n9_eta 0.15), which is what makes waste-heat steam cheap.
s.t. whr_pool_alloc{t in T}:
    whr_gas_to_power[t] + whr_gas_to_steam[t] <= whr_available_gas[t] * n9_whr[t];  #eq81b

# WHR-STUDY TOGGLE (this copy only): 1 = CCS regen steam may draw on the
# waste-heat pool (default, the integrated system); 0 = BOILER-ONLY -- the
# pool serves power exclusively and all regen steam must come from the
# gas-fired backup boiler. Sweep with `let whr_ccs_integration := 0;`.
