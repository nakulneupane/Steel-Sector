# ============================================================================
# ABATEMENT-ANATOMY sweep template -- one POLICY scenario per run.
# Seven-bar figure: Baseline, EF1.6, EF1.8, S4, S8, RL, RH -- each policy bar
# shows actual cumulative CO2 (empty) + its abatement vs the COMMON baseline
# broken into H2 / CCS / scrap / NG / grid / others slices. Defaults (central): NG 10,
# scrap 6%, MEDIUM ramp (15 Mt / 6 Mt), cap 1.8, H2 2030, thetas 0.5 -- each
# scenario perturbs ONE dial (EF1.8 therefore equals the central case).
# abatement.bat substitutes SCENLABEL / EFVAL / SCRAPVAL / RAMPVAL / H2REFVAL.
# Yearly rows appended to results/abatement_yearly.csv; run from THIS folder.
# ============================================================================
reset;
...
# Per-route GROSS emissions (scope1 route + its scope2 electricity, NO capture
# subtracted). Lets the pivot re-reference the H2/scrap/NG wedges to the baseline
# average intensity and add an NG-DRI slice (sigma_base parity with Transition).
