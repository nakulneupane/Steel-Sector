# ============================================================================
# ABATEMENT anatomy -- COMMON BASELINE run (frozen-structure BAU reference).
# One shared reference for ALL seven bars (default 6% scrap availability;`n# a per-scenario baseline would be infeasible at S4). Same backdrop otherwise (thetas 0.5, Medium ramp irrelevant, NG 10,
# scrap growth 6%) but with the Baseline definition (mirrors Baseline/):
#   no hydrogen (debut 2100), no CCS, emission cap inert, build slabs inert,
#   2025 route shares frozen for all years, AGGREGATE scrap share frozen at
#   the 2025 calibration (~24.31% of steel; allocation free),
#   emission_monotonic dropped, presolve_eps for the collinear share pins.
# Appends one row per year to results/abatement_yearly.csv; run from THIS folder.
# ============================================================================
reset;
...
# frozen 2025 structure (init_* pins first(T); these extend it)
...
# Per-route GROSS emissions (see abatement_scenario.mod). Baseline has no H2/CCS
# and no scrap-EAF route, but BF-BOF / coal-DRI / NG-DRI feed sigma_base and the
# per-route validation identity.
