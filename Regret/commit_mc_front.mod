# ============================================================================
# COMMIT-MC -- PARETO FRONTIER solve (panel c, multi-world): the
# cost-emissions tradeoff of a COMMITTED-WORLD CLOUD. H2 is committed and
# arrives at H2YEARVAL (= the world's cloud); the emission target
# avg_emi = AVGEMIVAL is swept {1.4 ... 2.2}; the market world is
# tokenized (same draws as commit_mc_run.py). Flexible optimum, same
# relaxations as commit_mc_case.mod; elastic cap, so unreachable targets
# report their Pareto floor (achieved intensity > target, slack > 0).
...
# same relaxations as commit_mc_case.mod (comparability)
drop min_util_bof;
drop min_util_cdri;
drop min_util_ngdri;
drop min_util_h2dri;
