# ============================================================================
# COMMIT study -- PLAN solve: the program signed in 2025 under an H2
# COMMITMENT YEAR (token H2YEARVAL): "we commit for hydrogen by <year>, and
# everything is planned accordingly; hydrogen is fully matured by 2050."
#
# Belief world = the STUDY BACKDROP (fixed for plans, PF and evaluations
# alike; only the realized H2 year differs):
#   theta_tech = 0.5, theta_grid = theta_ccs = 0.5 (central), scrap 6%/yr,
#   NG $10, coking coal MID, natural gas SHOCK, avg_emi 1.8, Medium ramp.
# Same relaxations as commit_case.mod (monotonic dropped, elastic cap +
# imports) so all costs are comparable; slack > 0 means the planner
# KNOWINGLY signs a program that misses the budget (late commitments).
# Dumps the 13 committed build trajectories to PLANOUT + PLANROW to stdout.
# ============================================================================
reset;
...
# same relaxations as commit_case.mod (comparability)
drop min_util_bof;
drop min_util_cdri;
drop min_util_ngdri;
drop min_util_h2dri;
