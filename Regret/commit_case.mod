# ============================================================================
# COMMIT study -- evaluation / epoch template. Port of Plots/Regret's
# pathway_case.mod (FLOOR commitments + FINANCIAL H2 supply, HANDOFF section 3)
# to the fixed-backdrop commitment experiment. Driven by commit_run.py.
#
# Tokens:
#   H2READYVAL  the H2 debut year THIS SOLVE plans with:
#               - PF / no-recourse / final-epoch evals: the REALIZED year
#               - recourse epoch A (2035): the planner's BELIEF at 2035
#                 (realized if arrived by 2035, else the falsified commitment
#                 rolled forward -- rule in commit_run.py)
#   TSTARVAL    builds strictly before TSTAR are floored to the committed
#               plan (2051 = no recourse; 2035 / 2045 = recourse epochs;
#               2025 = PF, plan_null)
#   PLANFILE    committed_build_* lets (a plan_c<Y>.mod, or the assembled
#               program+epoch-A history for the 2045 epoch)
#   DUMPOUT     file receiving this solve's builds for t in
#               [TSTARVAL, TSTARVAL+9] (used by epoch A to hand its executed
#               2035-2044 window to the 2045 epoch; ignored elsewhere)
#
# Backdrop fixed for ALL solves (= commit_plan.mod): theta_tech = theta_grid
# = theta_ccs = 0.5 (central), scrap 6%/yr, NG $10, coal MID, gas SHOCK,
# cap 1.8, Medium ramp. Only the H2 year differs between solves.
# Same relaxations as the Regret family: monotonic + min-util dropped,
# elastic cap (PEN 5000 undiscounted, reported costs EXCLUDE the penalty),
# elastic imports ($20000 objective / $650 reporting).
# ============================================================================
reset;
...
# --- H2 debut for THIS solve (realized or epoch belief; see header) ---
let ng_h2_start_year := H2READYVAL;
let h2_peak_year := ng_h2_start_year + 5;

# --- study backdrop (fixed) ---
let theta_tech := 0.5;
let theta_grid := 0.5;
let theta_ccs  := 0.5;
...
# --- committed plan (floors before TSTAR; H2 supply financial below) --------
...
# --- H2-supply commitment is FINANCIAL, not physical (HANDOFF section 3) ----
# Deployment obeys the realized world's native envelope exactly like the
# benchmark; the signed capex is paid on at least the committed units
# (sunk if unbuilt), charged in the objective AND the reported PV cost.
...
# --- min-utilisation floors dropped (all solves alike) ----------------------
drop min_util_bof;
drop min_util_cdri;
drop min_util_ngdri;
drop min_util_h2dri;
...
# --- elastic demand: imports at a premium -----------------------------------
...
# --- post-solve: stranding of the COMMITTED (pre-TSTAR) program --------------
...
# REGROW: status, PV cost (imports at parity, incl. financial-H2 payments,
# EXCL shortfall penalty, $B); LCOP per delivered tonne (PV $/t); emission
# shortfall (Mt CO2 over the 14.003-Gt budget); cumulative imports (Mt);
# cumulative capture (Mt); committed capex ($B); stranded total + six classes
# ($B: conventional / H2-DRI / H2 supply / CCS / scrap chain / other chains);
# ACHIEVED cumulative intensity (tCO2/t domestic steel -- emission-regret
# input: miss = max(0, intensity - 1.8)).
...
# --- dump this solve's builds for [TSTAR, TSTAR+9] (epoch handoff) -----------
